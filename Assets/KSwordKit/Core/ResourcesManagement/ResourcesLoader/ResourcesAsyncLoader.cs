                    ...
                    if (ResourcesManager.Instance.ResourceObjectPath_ResourceObjectDic.ContainsKey(assetPath))
                    {
                        ...
                        System.Action abloadedAction = () => {
                            ...
                            if (ro.IsScene && !isLoadScene)
                            {
                                ...
                                ResourcesManager.Instance.NextFrame(
                                    () => asyncAction(
                                        true,
                                        1,
                                        ResourcesManager.KSwordKitName + ": 资源加载失败! 该资源是场景资源，但是请求加载非场景资源，因此无法加载! 请使用 `KSwordKit.Core.ResourcesManagement.ResourcesManager.LoadSceneAsync` 相关的API再次尝试。\n参数 assetPath=" + assetPath,
                                        null
                                    )
                                );
                                return;
                            ...
                            if (ro.IsScene && isLoadScene)
                            {
                                if(!_TypeIsSceneInfo)
                                {
                                    Debug.LogWarning(ResourcesManager.KSwordKitName + ": 应当提供类型 `KSwordKit.Core.ResourcesManagement.SceneInfo`；当前操作将以默认方式继续加载。");
                                }
                                T _sceneinfo = null;
                            ...
                            if(!ro.IsScene && isLoadScene)
                            {
                                ...
                                ResourcesManager.Instance.NextFrame(
                                    () => asyncAction(
                                        true,
                                        1,
                                        ResourcesManager.KSwordKitName + ": 资源加载失败! 该资源不是场景资源，但是请求加载场景资源，因此无法加载! 请使用 `KSwordKit.Core.ResourcesManagement.ResourcesManager.LoadAssetAsync` 相关的API再次尝试。\n参数 assetPath=" + assetPath,
                                        null
                                    )
                                );
                                return;
