...
namespace KSwordKit.Core.ResourcesManagement
{
	...
	public interface IResourcesAsyncLoader
	{
		...
		/// <param name="asyncAction">异步回调；参数是异步结果，内部包含进度、错误信息、加载结果等内容；</param>
		void LoadAsync<T>(string assetPath, System.Action<ResourcesRequestAsyncOperation<T>> asyncAction) where T : UnityEngine.Object;
		/// <summary>
		/// 给定的路径（文件夹或文件）中加载所有资源
		...
		/// <param name="assetPath">给定的资源路径（文件夹或文件）</param>
		/// <param name="asyncAction">异步回调；参数是异步结果，内部包含进度、错误信息、加载结果等内容；</param>
		void LoadAllAsync<T>(string asstPath, System.Action<ResourcesRequestAsyncOperation<T>> asyncAction) where T : UnityEngine.Object;
		/// <summary>
		/// 给定的路径（文件夹或文件）中加载所有资源
		...
		/// <param name="deepResources">指示函数是否遍历子目录所有资源</param>
		/// <param name="asyncAction">异步回调；参数是异步结果，内部包含进度、错误信息、加载结果等内容；</param>
		void LoadAllAsync<T>(string asstPath, bool deepResources, System.Action<ResourcesRequestAsyncOperation<T>> asyncAction) where T : UnityEngine.Object;
	}
}
